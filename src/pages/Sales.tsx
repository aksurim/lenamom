...
type SaleDataForPrint = { saleId: number; saleCode: string; customer: Customer; paymentMethod: PaymentMethod; saleDetails: any; items: SaleItem[] };
...
// --- MODAL PÓS-VENDA (ATUALIZADO PARA WEBUSB) ---
function PostSaleModal({ saleData, onClose }: { saleData: SaleDataForPrint | null; onClose: () => void; }) {
    ...
    const handlePrintReceipt = async () => {
        ...
        try {
            ...
            toast.info("Por favor, selecione a impressora USB para imprimir o cupom.");
            await sendTsplOverUsb(tsplCommand);
        } catch (error: any) {
            console.error("Falha ao imprimir cupom via USB:", error);
        } finally {
            setIsPrinting(false);
        }
    };
    ...
    const handlePrintPdf = async () => {
        ...
        try {
            await generateStandardPdf({
              ...
              drawContent: (doc, startY) => { /* ... Lógica de desenho do PDF A4 ... */ }
            });
        } catch (error) {
            ...
            <DialogContent>
                ...
                <DialogFooter className="pt-4 flex-col sm:flex-row sm:justify-end gap-2">
                    ...
                    <Button onClick={handlePrintReceipt} disabled={isPrinting}>
                        {isPrinting ? "Imprimindo..." : "Imprimir Cupom (USB)"}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
  ...
  return (
    <>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
        {/* ... Restante do JSX da página de vendas ... */}
      </div>
